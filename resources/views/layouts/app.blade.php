...
<head>
    ...
    <title>@yield('title', 'SparZinsVergleich24 - Sparzinsen, Geldanlage & Finanznachrichten')</title>
    <meta name="description" content="@yield('meta_description', 'Maximieren Sie Ihre Zinserträge mit SparZinsVergleich24. Berichte und Analysen zu Anlagestrategien, Festgeld, Tagesgeld und Vermögensaufbau.')">
...
<body class="flex flex-col min-h-screen bg-slate-100 text-slate-900 font-sans antialiased">
    ...
    <div class="bg-slate-950 text-slate-300 text-[11px] border-b border-slate-800 py-1.5 px-4">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            <div class="flex items-center space-x-3 text-xs">
                ...
                <span class="text-slate-400 hidden sm:inline">SparZinsVergleich24 Portal</span>
                <span class="text-slate-700">|</span>
                <span class="text-orange-400 font-bold flex items-center text-[11px]">
                    <span class="w-2 h-2 rounded-full bg-orange-500 mr-1.5 animate-pulse"></span>
                    <span>Spar-Redaktion Aktiv</span>
                </span>
            </div>
            ...
            <div class="flex items-center space-x-3 text-[11px]">
                ...
                <span class="text-slate-700">•</span>
                <a href="{{ route('impressum') }}" class="text-orange-400 hover:text-orange-300 font-bold underline transition-colors">Impressum</a>
            </div>
        </div>
    </div>
    ...
    <div class="bg-slate-900 text-white text-xs py-2 border-b border-slate-800 overflow-x-auto no-scrollbar shadow-inner">
        <div class="max-w-7xl mx-auto px-4 flex items-center space-x-6 whitespace-nowrap">
            <span class="bg-indigo-700 text-white text-[10px] font-extrabold px-2 py-0.5 rounded uppercase tracking-wider flex items-center flex-shrink-0 shadow">
            ...
            <div class="flex items-center space-x-2">
                ...
                <span class="font-bold text-orange-400">3,75%</span>
            </div>
            <span class="text-slate-700">|</span>
    ...
    <header class="bg-white border-b border-slate-300 shadow-sm sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 py-3 sm:py-4">
            <div class="flex items-center justify-between">
                ...
                <div class="hidden lg:block text-xs text-slate-500 space-y-0.5 border-l-2 border-indigo-600 pl-3">
                    <div class="font-bold text-slate-900 uppercase tracking-wider">SparZinsVergleich24</div>
                    <div>Vermögens- & Sparmagazin</div>
                ...
                <div class="flex items-center space-x-2">
                    <a href="{{ route('news.index') }}" class="hidden sm:inline-flex px-4 py-2 bg-indigo-700 hover:bg-indigo-800 text-white font-extrabold text-xs rounded-lg shadow-sm transition-all">
                        Nachrichten Lesen
                    </a>
        ...
        <div class="bg-slate-950 text-white border-t border-b border-slate-900">
            <div class="max-w-7xl mx-auto px-4">
                <nav class="flex items-center space-x-1 overflow-x-auto py-1.5 no-scrollbar text-xs font-bold uppercase tracking-wider whitespace-nowrap">
                    <a href="{{ route('home') }}" class="px-3 py-2 rounded hover:bg-slate-800 transition-colors {{ request()->routeIs('home') ? 'bg-indigo-700 text-white' : 'text-slate-300' }}">
                        Startseite
                    </a>
                    ...
                    <a href="{{ route('kontakt') }}" class="px-3 py-2 rounded hover:bg-slate-800 text-slate-300 hover:text-white transition-colors {{ request()->routeIs('kontakt') ? 'bg-indigo-700 text-white' : '' }}">
                        Kontakt
                    </a>
                    <a href="{{ route('impressum') }}" class="px-3 py-2 rounded hover:bg-slate-800 text-orange-400 hover:text-orange-300 transition-colors {{ request()->routeIs('impressum') ? 'bg-indigo-700 text-white' : '' }}">
                        Impressum
                    </a>
                </nav>
            </div>
        ...
        <div id="mobile-menu-drawer" class="hidden bg-slate-900 text-white border-b border-slate-800 px-4 py-4 space-y-3">
            ...
            <a href="{{ route('kontakt') }}" class="block px-3 py-2 rounded text-sm font-medium text-slate-200">Kontakt</a>
            <a href="{{ route('impressum') }}" class="block px-3 py-2 rounded text-sm font-bold text-orange-400 bg-slate-800/80">Impressum</a>
        </div>
    </header>
    ...
    <footer class="bg-slate-950 text-slate-400 text-xs border-t-4 border-indigo-600 pt-12 pb-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                <div class="space-y-3">
                    ...
                    <p class="text-slate-400 leading-relaxed text-xs font-serif">
                        Unabhängiges Finanzmedien-Portal für Sparzinsen, Festgeld und Geldanlage.
                    </p>
                </div>
                <div>
                    <h4 class="text-xs font-bold text-white uppercase tracking-wider mb-3 border-b border-slate-800 pb-1">Rubriken</h4>
                    <ul class="space-y-2">
                        <li><a href="{{ route('news.index', ['category' => 'Wirtschaft & Konjunktur']) }}" class="hover:text-orange-400">Wirtschaft & Konjunktur</a></li>
                        <li><a href="{{ route('news.index', ['category' => 'Börse & Märkte']) }}" class="hover:text-orange-400">Börse & Märkte</a></li>
                        <li><a href="{{ route('news.index') }}" class="hover:text-orange-400">Finanznachrichten Archiv</a></li>
                    </ul>
                </div>
                <div>
                    <h4 class="text-xs font-bold text-white uppercase tracking-wider mb-3 border-b border-slate-800 pb-1">Schwerpunkte</h4>
                    <ul class="space-y-2">
                        <li><a href="{{ route('news.index', ['category' => 'Politik & EZB']) }}" class="hover:text-orange-400">EZB Leitzins-Entscheidungen</a></li>
                        <li><a href="{{ route('news.index', ['category' => 'Immobilien & Zinsen']) }}" class="hover:text-orange-400">Bauzinsen & Markt</a></li>
                        <li><a href="{{ route('kontakt') }}" class="hover:text-orange-400">Kontakt & Anfragen</a></li>
                    </ul>
                </div>
                <div class="bg-slate-900 p-4 rounded-xl border border-slate-800 space-y-2">
                    <h4 class="text-xs font-bold text-white uppercase tracking-wider text-orange-400">Impressum Angaben</h4>
                    ...
                    <div class="pt-2 space-x-2">
                        <a href="{{ route('impressum') }}" class="inline-block text-orange-400 font-bold hover:underline">Impressum &rarr;</a>
            ...
            <div class="border-t border-slate-900 pt-6 text-center text-slate-400 text-[11px] flex flex-col md:flex-row justify-between items-center">
                <p>&copy; 2026 {{ config('app.name', 'SparZinsVergleich24') }}. Alle Rechte vorbehalten. Inhaber: Oskar Ehrenfried Heinrich.</p>
                <div class="space-x-4">
